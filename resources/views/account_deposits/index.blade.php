                        ...
                        <tr class="hover:bg-background/30 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-right">
                                ...
                                <form action="{{ route('account-deposits.destroy', $deposit) }}" method="POST" class="inline-block">
                                    @csrf @method('DELETE')
                                    <button type="button" @click="$dispatch('open-delete-modal', { action: '{{ route('account-deposits.destroy', $deposit) }}', message: 'Are you sure you want to delete this deposit record? This action cannot be undone.' })" class="w-8 h-8 rounded-lg flex items-center justify-center text-muted hover:bg-danger/10 hover:text-danger transition-colors" title="Delete Deposit">
