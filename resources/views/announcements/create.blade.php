<x-app-layout>
    ...
    <div class="px-6 py-8 max-w-3xl mx-auto" x-data="{ level: '{{ old('visibility_level', $levels[0] ?? 'national') }}' }">
        <div class="bg-surface rounded-2xl shadow-sm border border-border overflow-hidden">
            <div class="p-6 border-b border-border bg-background/50">
                <h2 class="text-lg font-black text-text">New Announcement</h2>
                <p class="text-sm text-muted mt-1">Create a new announcement visible to the selected audience.</p>
            </div>
            ...
            <form action="{{ route('announcements.store') }}" method="POST" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="6" required
                              class="w-full px-4 py-3 rounded-xl bg-background border border-border text-text focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors resize-y"
                              placeholder="Write the announcement content...">{{ old('content') }}</textarea>
                ...
                <div>
                    ...
                    @error('visibility_level') <p class="text-danger text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-border">
                    <a href="{{ route('announcements.index') }}" class="px-6 py-2.5 rounded-xl border border-border text-muted font-bold hover:bg-background transition-colors text-sm">Cancel</a>
